...
public class CommentAppService : BlogSampleAppService, ICommentAppService
{
    ...
    public async Task<List<CommentWithRepliesDto>> GetHierarchicalListOfPostAsync(Guid postId)
    {
        ...
        foreach (var commentDto in comments)
        {
            ...
            {
                var creatorUser = await _userLookupService.FindByIdAsync(commentDto.CreatorId.Value);
